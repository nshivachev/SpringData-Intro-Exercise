package com.softuni.usersystem.annotations.password;

import com.softuni.usersystem.annotations.AnnotationsUtil;
...
public class PasswordValidator implements ConstraintValidator<Password, CharSequence> {
    ...
    public boolean isValid(final CharSequence value, final ConstraintValidatorContext context) {
        ...
        if (!PATTERN_LOWER.matcher(password).find() && this.hasLower) {
            AnnotationsUtil.setErrorMessage(context, Constants.PASSWORD_SHOULD_CONTAIN_LOWER_CASE_LATTER);
            return false;
        }

        if (!PATTERN_UPPER.matcher(password).find() && this.hasUpper) {
            AnnotationsUtil.setErrorMessage(context, Constants.PASSWORD_SHOULD_CONTAIN_UPPER_CASE_LATTER);
            return false;
        }
